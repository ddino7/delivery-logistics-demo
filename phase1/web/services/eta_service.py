...
class EtaService:
    ...
    def predict(self, shipment: Dict[str, Any], route: Dict[str, Any]) -> Tuple[float, str]:
        ...
        if self.model:
            try:
                ...
                # fall back if model inference fails
                pass
    ...
    def _build_features(self, shipment: Dict[str, Any], route: Dict[str, Any]):
        # Minimal feature vector; expand when model supports more signals
    ...
    def _heuristic_eta(self, route: Dict[str, Any], shipment: Dict[str, Any]) -> float:
        """
        Simple fallback: max(planned_time, distance / avg_speed) with small
        handling buffer influenced by weight.
        """
